...
void science(int x) {
    ...
    srand(time(0));
    int totalAsked = (count < 10) ? count : 10; // Fix: Don't ask more questions than available
    int score = 0;
    int totalPoints = 0;
    ...
    for (int i = 0; i < totalAsked; i++) {
        ...
        while (!questionAnswered) {
            ...
            if (!inputReceived) {
                ...
                incorrect[wrongcount++] = q; // Fix: Add to incorrect list
                Sleep(2000);
            ...
            if (anss == 0) {
                ...
                // Fix: Better 50-50 implementation
    ...
    if (replay == 1) {
        science(x);
        return; // Fix: Return after recursive call
    }
